...
- 对话持久化：主对话、副对话和冻结的论文正文写入 `{dataDir}/zoterochat.sqlite`。重开同一 PDF 会恢复记录。「新对话」归档旧会话。
- 库文件用绝对路径打开（外部库），避免被当成内部库去做完整性检查而卡住。

## 0.1.11 — 2026-09-22

- **针对选段提问**：助手回复上划词或点操作条，打开盖在主对话上的 overlay；问完回到原处。复用论文前缀，不写进主时间线。划词产生的副对话在原文下加下划线（单击进入，拖选仍可复制）。见 [adr/0009](adr/0009-aside-overlay.md)。
- **PDF 划词翻译**：弹窗显示译文（默认同 Google，可换 DeepL / Google Cloud / Azure / 对话模型）。textarea 可拖拽改大小；A− / A+ 或设置里改译文字号。「解释选区」仍在。沙箱里禁止直接 `new AbortController()`，避免整块弹窗消失。
- 设置页：「保存并刷新」、去掉重复的 ZoteroChat 标题、「翻译」一节与上方连接区分开。
- Tooltip / 弹出列表 / 划词条使用不透明底，避免叠在正文上。
- 字号观察器与 `Zotero.Prefs.set(..., true)` 同一 pref 分支。
