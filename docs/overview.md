...
```
[0]     system          固定常量（含「用 LaTeX 写公式」）。禁止插值日期 / 模型名 / UI 语言 / 论文标题
...
[n+1]   user  CURRENT   <turn-directives> 语言锁（放最前）
                        + 可选 <selection> USER_SELECTED_PASSAGE + 高亮正文
                        + <question>
```

- `[1][2]` 构成**闭合前缀单元**——满足 DeepSeek 的完整单元匹配规则，同时让第 2 轮
  起 `[0..2]` 稳定命中。
- 一切动态偏好（回答语言、当前划词）放 `[n+1]`，**绝不进 system**。语言锁在当前轮
  最前，避免被长问题淹没。
- LaTeX 输出约定写在冻结的 system 里（对用户不可见，且不随语言变化）。
- 划词只属于当前轮。用户叉掉芯片后，在重新划词之前不得再把该段塞回上下文
  （`dismissSelection` + 面板侧以芯片状态为发送真相，禁止回落到阅读器 live 选区）。
- 「针对选段提问」是盖在主对话上的 overlay，不改 `[0][1][2]`；副对话快照分叉时的主历史，
  引用片段只在首轮后缀。见 [adr/0009](adr/0009-aside-overlay.md)。
- PDF 划词弹窗可显示译文（默认 Google）。翻译不进入对话、不改冻结前缀。
- OpenAI 的 `prompt_cache_key` 尚未发送；DeepSeek 当前只靠前缀字节匹配。
...
| 缓存   | 无任何处理，不读 `cached_tokens`，无 `prompt_cache_key`                            | 前缀分层 + PrefixLedger；读 `cached_tokens` / `prompt_cache_hit_tokens`。`prompt_cache_key` 尚未发送 |
...
| XHTML 禁止 innerHTML  | 面板是 XHTML，MathML 会抛 InvalidCharacterError                             | [0007](adr/0007-xhtml-html-inject.md) |
| 选区只在当前轮        | 高亮易变，进前缀会打穿缓存；芯片 dismiss 才是发送真相                       | [0008](adr/0008-selection-suffix.md)  |
