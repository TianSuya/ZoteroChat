...
> **早期版本：** 对话仅保存在内存中，重启后无法恢复。侧栏重建（包括部分标签页切换）也会开始新对话。离开当前会话前，请复制需要保留的回答。

## 功能

- **围绕当前论文讨论。** 从概括全文、讲解方法、局限或关键术语开始，继续追问。
- **针对段落提问。** 划词后将选区附到下一轮，或在阅读器弹窗点击「解释选区」。
...
| 对话历史 | 仅保存在内存中，不支持重启恢复、编辑已提交消息或对话分支。侧栏重建可能重置历史。                                                                      |
